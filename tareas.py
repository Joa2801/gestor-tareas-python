...
def normalizar_tareas():
    ...
    for tarea in tareas:
        # Unifica todos los nombres de campo a "descripción"
...
def mostrar_tareas(tareas):
    ...
    for tarea in tareas:
        # Busca en todos los posibles nombres de campo
        ...
        table.add_row(
            str(tarea["id"]),
            tarea["título"],
            desc,  # Usa la descripción encontrada
...
def mostrar_tareas(tareas):
    ...
    for tarea in tareas:
        # Si existe 'descripcion' (sin tilde), lo cambiamos
        if 'descripcion' in tarea:
            tarea['descripción'] = tarea.pop('descripcion')
            modificado = True
            
        # Si existe 'Descripción' (con tilde y mayúscula), lo cambiamos
